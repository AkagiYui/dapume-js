/** 中文词条。结构与 en.ts 保持一致。 */
export const zh = {
  app: {
    title: '打谱么',
    tagline: '线性乐谱在线编辑与播放',
  },
  nav: {
    tutorial: '教程',
    guide: '规则与语法',
    developers: '开发者',
    workbench: '工作台',
    tryNow: '马上尝试',
  },
  dev: {
    title: '在你的项目中使用 dapume-js',
    subtitle:
      'dapume-js 是一个零依赖的 TypeScript 库，可在 Node 与浏览器中把线性乐谱解析为乐谱对象，并渲染为 MIDI。本站正是基于它构建的示例。',
    installTitle: '安装',
    installDesc: '用你喜欢的包管理器安装：',
    apiTitle: '核心 API',
    apiDesc:
      '库暴露两个核心函数：parse() 把 dapume 文本解析为乐谱对象，toMidi() 把乐谱对象渲染为 MIDI 文件字节（Uint8Array）。',
    nodeTitle: '在 Node 中使用',
    nodeDesc: '解析并渲染为 MIDI，然后写入文件：',
    browserTitle: '在浏览器中使用',
    browserDesc: '得到 MIDI 字节后，用 Blob 触发下载：',
    moreTitle: '更多工具函数',
    moreDesc:
      'render() 一步完成「解析 + 渲染」；tokenize() 返回语法高亮词法单元；activeNotesAt() 返回正在发声的音符；activeEventsAt() 还会返回当前休止符；paramsAt() 返回某时刻生效的调号与速度。',
    typesTitle: '类型一览',
    vscodeTitle: 'VSCode 扩展',
    vscodeDesc:
      '本仓库还提供一个 VSCode 扩展：为 .dapume 文件提供语法高亮，并在编辑器标题栏提供「渲染为 MIDI」按钮（内部即调用 dapume-js）。可在 GitHub Actions 的构建产物中下载 .vsix 安装。',
    cta: '到工作台试试',
    repo: 'GitHub 仓库',
    originalRepo: '原 Python 项目',
    apiRef: 'API 参考',
    marketplace: 'VSCode 扩展',
    httpTitle: 'HTTP 接口',
    httpDesc:
      '线上站点还提供三个由 dapume-js 驱动的 HTTP 接口（Cloudflare Pages Functions）；相同请求体会被缓存，render 在响应头返回音符数 / 音轨数 / 时长。',
    httpSameOrigin: '出于安全考虑，这些接口仅允许同域（本站）调用。',
    tryTitle: '在线测试',
    tryDesc: '输入 dapume 文本，调用线上接口查看实时返回。',
    tryInput: 'dapume 文本',
    tryRun: '发送请求',
    tryRunning: '请求中…',
    tryResult: '响应',
    tryError: '请求失败',
    tryDownload: '下载 MIDI',
  },
  settings: {
    title: '设置',
    theme: '外观',
    light: '浅色',
    dark: '深色',
    system: '跟随系统',
    themeColor: '主题色',
    language: '语言',
  },
  color: {
    default: '默认',
    blue: '蓝色',
    violet: '紫色',
    green: '绿色',
    rose: '玫红',
    orange: '橙色',
    amber: '琥珀',
  },
  common: {
    play: '播放',
    stop: '停止',
    loading: '加载中…',
    confirm: '确定',
  },
  guide: {
    heroTitle: '打谱么 · 线性乐谱',
    heroSubtitle: '用纯文本书写简单乐谱，一键渲染为 MIDI。下面带你认识它的规则与语法。',
    heroCta: '马上尝试',
    startTutorial: '新手教程',
    tocTitle: '目录',
    playExample: '播放示例',
    backToTop: '回到顶部',
    installApp: '安装应用',
  },
  tutorial: {
    heroTitle: '一步步学会线性乐谱',
    heroSubtitle: '从一个音符开始，循序渐进，最后写出一段双手多轨谱。每一步都能点击播放听效果。',
    openWorkbench: '到工作台动手写',
    fullReference: '完整语法手册',
    outro: '这些就是全部基础啦——去工作台把它们组合成你自己的曲子吧！',
  },
  update: {
    available: '有新版本可用',
    refresh: '刷新',
    dismiss: '忽略',
  },
  manager: {
    title: '我的乐谱',
    subtitle: '在浏览器中管理你的所有 dapume 乐谱',
    new: '新建乐谱',
    untitled: '未命名乐谱',
    empty: '还没有乐谱，点击「新建乐谱」开始。',
    open: '打开',
    rename: '重命名',
    delete: '删除',
    autoOpen: '直接访问时自动打开上次编辑的乐谱',
    updated: '更新于',
    exported: '导出于',
    notes: '音符',
    sortBy: '排序',
    sortRecent: '最近',
    sortName: '名称',
    sortCreated: '创建',
    sortReverse: '正序 / 倒序',
    confirmDelete: '确定删除「{{ title }}」吗？此操作不可撤销。',
    renamePrompt: '新名称：',
    renameTitle: '重命名乐谱',
    deleteTitle: '删除乐谱',
    cancel: '取消',
    save: '保存',
    share: '分享',
    import: '导入',
    shareTitle: '分享乐谱',
    shareDesc: '用另一台设备扫描这组动态二维码即可导入；也可导出为动图发送。',
    exportAnim: '导出动图',
    exportUnsupported: '当前浏览器不支持该格式',
    exporting: '导出中…',
    importTitle: '导入乐谱',
    importScan: '把摄像头对准动态二维码，直到下方分段全部点亮；也可上传二维码动图。',
    importUpload: '上传图片 / 动图',
    importCameraError: '无法访问摄像头，可改用上传图片。',
    importDecodeError: '未识别到有效的乐谱二维码。',
  },
  workbench: {
    play: '播放',
    pause: '暂停',
    stop: '停止',
    replay: '从头播放',
    followPlayback: '钢琴卷帘跟随播放进度',
    keepLine: '保持当前演奏行可视',
    stickyParam: '参数行粘性置顶',
    smoothScroll: '平滑滚动',
    downloadMidi: '导出 MIDI',
    downloadDpm: '导出 dapume',
    downloadQr: '二维码分享',
    editorTitle: '乐谱编辑器',
    hintsTitle: '语法提示',
    controlsTitle: '操作',
    pianoRollTitle: '钢琴卷帘',
    visualizer: '谱面可视化',
    jianpuTitle: '简谱',
    jianpuHint: '点击音符定位 · 每行 4 小节',
    jianpuVoice: '声部 {{ number }}',
    jianpuChordTrack: '和弦声部',
    jianpuChordShort: '和',
    jianpuRestLabel: '休止符',
    jianpuEndMark: '终止线',
    pianoOrientation: '卷帘朝向（横向 / 纵向）',
    pianoKeyDir: '琴键方向（高音 ↔ 低音）',
    pianoKeyboardPos: '琴键位置（左·底 ↔ 右·顶）',
    pianoJudgeLine: '判定线（贴键盘落键 ↔ 悬浮·纯瀑布）',
    pianoCenter: '编辑器与谱面预览并排（左右布局）',
    prevNote: '上一个音符',
    nextNote: '下一个音符',
    prevLine: '上一行',
    nextLine: '下一行',
    delayDown: '减少视觉延迟',
    delayUp: '增加视觉延迟',
    visualDelay: '视觉延迟（适配无线耳机）',
    loadingPiano: '正在加载钢琴音色…',
    pianoReady: '音色就绪',
    audioError: '音色加载失败，请检查网络',
    emptyScore: '在左侧编辑器输入 dapume 文本，谱面将实时渲染于此',
    examples: '示例',
    loadExample: '载入示例',
    clear: '清空',
    notes: '音符数',
    tracks: '音轨',
    measures: '小节数',
    measureBeat: '第 {{ measure }} 小节 · 第 {{ beat }} 拍',
    metronomeOn: '开启节拍器',
    metronomeOff: '关闭节拍器',
    key: '调性',
    tempo: '速度',
    duration: '时长',
    playingLocked: '播放中，编辑已锁定',
    quickRef: '速查',
    help: '速查与示例',
    download: '导出',
    confirmOverwrite: '载入将覆盖当前乐谱内容，确定继续？',
  },
} ;
