 ...
 *
 * 标题链接目标用 HeaderConfigContext 注入（默认 `/docs`）：dapume-web 沿用默认；
 * 社区站把它设为 `/`（点击标题回社区首页）。右侧导航（TopNav）与导航锚点（NavA）均导出，便于社区外壳复用。
 */
...
/** 读取标题目标路由。 */
export function useTitleTo(): string {
  return useContext(HeaderConfigContext).titleTo;
}
  ...
  return (
    ...
    >
      <div class="mx-auto flex max-w-5xl items-center justify-between gap-3 px-4 py-3">
        <NavA to={titleTo} exact class="flex items-baseline gap-2" activeClass="">
